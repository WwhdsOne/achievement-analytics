...
- **只有速率限制：1 req/s（官方文档），按机器分桶**，没有日配额
- 2026-09-22 取消本项目自设的 1000/天上限：48,798 个待抓任务按 1000/天要 49 天，
  而官方限速 1 req/s 理论上限 86,400/天，1000 过于保守。**与 RAWG 不同——不存在按 key
  的跨机器配额账本**，多机并行时各机独立 1 req/s（聚合速率 = 机器数 × 1 req/s）
- 2026-09-22 曾整站开 Cloudflare challenge（403），当天已恢复；再遇到用
  `worker --exclude steamspy` 挂起，探测 200 后再放开
...
| `steamspy` | 49,900 款 | 49,900 | ≈ 13.9 小时 | ✅ **线性缩短**（只受 1 req/s 速率限制） |
