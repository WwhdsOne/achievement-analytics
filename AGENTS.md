...
```
crawler/    每个数据源一个模块（steam_api.py / steamspy.py）
cleaning/   清洗、成就名称映射与反作弊，输入 data/raw，输出 data/processed
...
- 爬虫密钥放 `.env`（`.env` 不入 git），读取统一走 `crawler/config.py`

## 数据规范

- `data/` 分层：`raw/`（原始，只读不改）→ `interim/`（中间）→ `processed/`（建模输入）
- **原始数据永不入库**（体积大且含个人信息），git 只跟踪处理后的样本与 schema
- schema 变更必须同步更新 `cleaning/schema.py` 并通知全组
- 玩家 ID 一律哈希化后再进入 processed 层
- **成就名称映射是硬性要求**：`GetGlobalAchievementPercentagesForApp` 与 `appdetails` 的 `name` 都是 API 内部名称，以它为键 JOIN 后才有 `displayName`；建模只消费映射后的标准成就数据文件，禁止直接用内部名称进报告
- 全局完成率 percent 随抓取批次漂移，建模必须绑定固定数据版本，记录抓取时间

## 爬虫规范（硬性）

- 限速：请求间隔 ≥ 1s，遵守各站 robots / ToS，只采公开数据
- 必须带缓存层（已爬过的 URL/UID/AppID 不重复请求），断点续爬
